...
import type {
  EnergyBaseline,
  ApiEnergyBaselineResponse,
} from "@/lib/energy-baseline/types";
...
const BASE_URL = "http://192.168.0.55:8080/SystemOptions";
  ...
  if (req.method === "GET") {
    try {
      const response = await fetch(
        `${BASE_URL}/GetEnergyBaselineMain.ashx?rows=1000&page=1&sidx=EbSgt&sord=asc`
      );
      ...
      const baselines = data.rows.map((row) => ({
        id: row.EbSgt,
